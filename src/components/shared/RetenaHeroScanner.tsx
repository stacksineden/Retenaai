...
const CAMPAIGN_SETS = [
  ...
  },
  {
    id: 2,
    category: "SLIPPERS",
    rawImage: "/assets/mockups/men_slippers/men_slippers_proto.JPG",
    assets: [
      "/assets/mockups/men_slippers/men_slippers11.png",
      "https://res.cloudinary.com/dyryfgjro/video/upload/v1763802767/vid1_xxbqb1.mp4",
      "/assets/mockups/men_slippers/men_slippers7.png",
      "/assets/mockups/men_slippers/men_slippers4.png",
    ],
  },
  {
    id: 3,
...
const RetenaInfinityEngine = () => {
  ...
  const renderAsset = (url: string, className: string) => {
    if (url.endsWith(".mp4")) {
      return (
        <video
          src={url}
    ...
    return <img src={url} alt="Asset" className={className} />;
  };
  ...
  return (
    <div className="relative w-full min-h-[850px] md:min-h-[950px] bg-[#000000] overflow-hidden flex flex-col items-center justify-center pt-14 md:pt-16 pb-4 md:pb-32">
      ...
      <div className="relative w-full max-w-[1000px] h-[450px] md:h-[600px] flex items-center justify-center mt-4">
        ...
        {/* === CENTER CORE === */}
        {/* FIX 1: BIGGER. Increased from w-28 to w-40 for mobile. */}
        <div
        ...
        {/* === SATELLITES (DENSE PACK FIX) === */}
        {/* STRATEGY:
            1. SIZE: Increased width significantly (w-28, w-32, w-40).
            2. DISTANCE: Decreased translation (80-95%) to keep them clustered tight.
        */}

        {/* SATELLITE 1: IG STORY */}
        {/* Size: w-28 (Big). Translate: 95% (Close) */}
        <div
          ...
          ${
            isExpanded && !isFading
              ? "-translate-x-[95%] -translate-y-[85%] md:-translate-x-[160%] md:-translate-y-[60%] opacity-100 rotate-[-6deg]"
              : "translate-x-0 translate-y-0 opacity-0 scale-50"
          }
        `}
        >
          {renderAsset(currentSet.assets[0], "w-full h-full object-cover")}
        ...
        {/* SATELLITE 2: VIDEO AD */}
        {/* Size: w-40 (Huge). Translate: 85% (Very Close) */}
        <div
          ...
          ${
            isExpanded && !isFading
              ? "translate-x-[85%] -translate-y-[90%] md:translate-x-[130%] md:-translate-y-[80%] opacity-100 rotate-[3deg]"
              : "translate-x-0 translate-y-0 opacity-0 scale-50"
          }
        `}
        >
          {renderAsset(currentSet.assets[1], "w-full h-full object-cover")}
        ...
        {/* SATELLITE 3: SQUARE SHOP */}
        {/* Size: w-32 (Big). Translate: 95% (Close) */}
        <div
          ...
          ${
            isExpanded && !isFading
              ? "-translate-x-[95%] translate-y-[80%] md:-translate-x-[180%] md:translate-y-[60%] opacity-100 rotate-[5deg]"
              : "translate-x-0 translate-y-0 opacity-0 scale-50"
          }
        `}
        >
          {renderAsset(currentSet.assets[2], "w-full h-full object-cover")}
        </div>

        {/* SATELLITE 4: POSTER */}
        {/* Size: w-32 (Big). Translate: 90% (Close) */}
        <div
          ...
          ${
            isExpanded && !isFading
              ? "translate-x-[90%] translate-y-[80%] md:translate-x-[140%] md:translate-y-[50%] opacity-100 rotate-[-4deg]"
              : "translate-x-0 translate-y-0 opacity-0 scale-50"
          }
        `}
        >
          {renderAsset(currentSet.assets[3], "w-full h-full object-cover")}
